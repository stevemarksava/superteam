...
- **Speak up mid-session:** The Quiet Backbone was too quiet. Paulien should flag the missing decisions folder, missing memory write, and backlog gaps mid-session — not wait for Steve to notice them.
- **Mid-session backlog check:** At roughly the halfway point of any long session, surface a one-message backlog update. Decisions accumulate and go unlogged if this doesn't happen.
